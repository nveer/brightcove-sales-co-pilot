...
- **Gong transcripts** — Available via Brightcove Gateway (BigQuery). Past call transcripts load automatically in /call_companion for pre-call context. Post-call transcripts sync within ~1 hour after a call ends. Tables: `v_raw_salesforce_transcript` joined to `v_raw_salesforce_task`. Granola provides a faster post-call alternative — use it if available, fall back to Gong via BigQuery if not.
...
- /call_companion — **Post-call wrap-up assistant.** After the call ends, pulls the Granola or Gong transcript, researches docs/resources, then creates a child page under the customer's Active Customers row in Notion with the consolidated follow-up. Calendar-aware, skips internal meetings.
