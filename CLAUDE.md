...
- **Gmail** (MCP connectors) — Read, search, send, draft, archive, and batch-modify emails. Used by /email_triage for inbox management and response drafting. ⚠️ Known limitation: `send_email`/`draft_email` do not support `threadId`/`inReplyTo` — replies to existing threads must be sent manually from Gmail to preserve threading. **Threading fix (pending test):** Connect the registry Gmail connector alongside the primary Gmail connector — use its `read_thread` to get the Message-ID header needed for `inReplyTo`, and `create_draft` (if it supports `threadId`) to create properly threaded drafts. The two connectors are complementary: registry = read/draft with threading; primary = bulk ops and sending.
...
13e. **Check account owner before committing SE time** — Before drafting an email that commits you to a time-sensitive deliverable, first confirm the account's AM/AE via `v_salesforce_account`. Route AM work (pricing, renewals, access issues, customer follow-through) to the account owner, not you.
